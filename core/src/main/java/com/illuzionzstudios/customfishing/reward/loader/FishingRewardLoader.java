...
public class FishingRewardLoader extends YamlFileLoader<FishingReward> {
	...
	public FishingReward loadYamlObject() {
		...
		try {
			...
			cause = "Could not load mobs";
			ArrayList<FishingItemLoader> mob = new ArrayList<>();
			// Parse custom items
			for (ConfigSection section : config.getSections("mob")) {
				mob.add(new FishingItemLoader(section));
			}
			builder.mob(mob);
